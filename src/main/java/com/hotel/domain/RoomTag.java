...
public record RoomTag(
    ...
    // =========================================================================
    // Room.initDefaultTags() 및 시스템 전반에서 참조하는 기본 태그 프리셋 상수
    // =========================================================================
    public static final RoomTag HIGH_FLOOR = new RoomTag(
            "HIGH_FLOOR", "고층", "10층 이상의 상층부 객실. 고층 전망, 뷰 선호", TagCategory.FLOOR, 15);

    public static final RoomTag LOW_FLOOR = new RoomTag(
            "LOW_FLOOR", "저층", "6층 이하 저층 객실. 보행 편의, 어르신/유아 동반", TagCategory.FLOOR, 15);

    public static final RoomTag NEAR_ELEVATOR = new RoomTag(
            "NEAR_ELEVATOR", "엘리베이터 인접", "엘리베이터와 가까워 이동이 편함", TagCategory.LOCATION, 20);

    public static final RoomTag AWAY_FROM_ELEVATOR = new RoomTag(
            "AWAY_FROM_ELEVATOR", "엘리베이터 이격", "엘리베이터와 멀리 떨어진 복도 안쪽 방. 소음 차단", TagCategory.LOCATION, 20);

    public static final RoomTag CORNER_ROOM = new RoomTag(
            "CORNER_ROOM", "코너룸", "건물 모퉁이 끝방. 2면 창문, 독립적 공간", TagCategory.VIEW, 15);

    public static final RoomTag QUIET_ZONE = new RoomTag(
            "QUIET_ZONE", "조용한 방", "소음 민감 고객, 아기 동반, 수면 방해 최소화", TagCategory.NOISE, 25);

    public static final RoomTag VIEW_TOKYO_TOWER = new RoomTag(
            "VIEW_TOKYO_TOWER", "도쿄타워 전망", "창문 밖으로 도쿄타워가 조망되는 객실", TagCategory.VIEW, 30);

    public static final RoomTag ACCESSIBLE = new RoomTag(
            "ACCESSIBLE", "배리어프리", "휠체어 이동 및 장애인/노약자 편의 설비 완비", TagCategory.AMENITY, 40);
}